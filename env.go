...
type Options struct {
	...
	CaptchaOptions struct {
		...
		AntiCaptcha uint8  `short:"c" long:"captcha" description:"решалка капчи\n0 - нейронка\n1 - RuCaptcha\n2 - вручную\n" default:"0" default-mask:"нейронка" choice:"0" choice:"1" choice:"2" hidden:"true"`
		Key         string `short:"k" long:"key" description:"ключ для API антикапчи" hidden:"true"`
		OcrServer   string `short:"o" long:"ocr-server" description:"API url нейронки" default:"http://127.0.0.1:7860/api/predict" hidden:"true"`
		Solve       bool   `long:"solve" description:"считать результат решения капчи" hidden:"true"`
